    ...
    def _get_hook(self, name, hidden_states):
        def hook_fn(module, input, output):
            if type(output) is tuple:
                 # For some modules, such as model.layers.{i}, output is a tuple with one element
                 # For some modules, such as attention intermediate layers, output is a "real" tuple: BEWARE
                 output = output[0]
